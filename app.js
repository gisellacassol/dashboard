...
const SERVICE_WORKER_URL = "./sw.js?v=11.11";
...
async function getTrainingPhotoRecords(categoryName = null) {
    ...
    const visibleYear = currentTrainingCalendarMonth.getFullYear();
    const visibleMonth = currentTrainingCalendarMonth.getMonth();
    const visibleStartDate = getLocalDateString(new Date(visibleYear, visibleMonth, 1, 12));
    const visibleEndDate = getLocalDateString(new Date(visibleYear, visibleMonth + 1, 0, 12));
    const filteredLocal = localRecords.filter(record =>
        (!categoryName || record.category === categoryName)
        && record.date >= visibleStartDate
        && record.date <= visibleEndDate
    );
    ...
    try {
        ...
        const { data: feedData, error: feedError } = await supabaseClient.functions.invoke("training-photo-feed", {
            body: { category_ids: visibleCategoryIds, start_date: visibleStartDate, end_date: visibleEndDate }
        });
        if (!feedError && Array.isArray(feedData?.photos)) {
            const cloudRecords = feedData.photos.map(item => ({
                ...
                thumbnail: item.thumbnail_signed_url || "",
        ...
        const { data, error } = await supabaseClient.from("training_photos")
            ...
            .gte("training_date", visibleStartDate)
            .lte("training_date", visibleEndDate)
            .order("created_at", { ascending: false });
        if (error) throw error;
        ...
        if (paths.length) {
            const { data: thumbnails, error: thumbnailsError } = await supabaseClient.storage.from("training-photos").createSignedUrls(paths, 3600, {
                transform: { width: 360, height: 360, resize: "cover", quality: 62 }
            });
            if (thumbnailsError) console.warn("Miniaturas otimizadas indisponíveis; usando as fotos normais.", thumbnailsError.message);
            else (thumbnails || []).forEach(item => thumbnailByPath.set(item.path, item.signedUrl));
        }
        const cloudRecords = visiblePhotoRows.map(item => ({
            ...
            thumbnail: thumbnailByPath.get(item.photo_path) || "",
...
async function renderTrainingReport() {
    ...
    // Mostra imediatamente o que já existe no aparelho. Rede e URLs assinadas
    // são atualizadas depois, sem manter o relatório inteiro em espera.
    ...
    const [, refreshedRecords] = await Promise.all([
        loadTrainingCompletionsForVisibleMonth(categoryName),
        getTrainingPhotoRecords(categoryName)
    ]);
...
function saveTrainingPhotoFeedCache(records) {
    ...
    const lightweightRecords = records.slice(0, 120).map(record => ({
        ...
        thumbnail: record.thumbnail || "", creatorAvatar: record.creatorAvatar, createdAt: record.createdAt
    }));
...
function prefetchRecentTrainingPhotos(records) {
    ...
    const recentPhotos = records.filter(record => (record.thumbnail || record.photo) && String(record.date || "").startsWith(monthPrefix)).slice(0, 6);
    const prefetch = () => recentPhotos.forEach(record => {
        const image = new Image();
        image.decoding = "async";
        image.src = record.thumbnail || record.photo;
    });
